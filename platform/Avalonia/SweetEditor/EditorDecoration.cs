...
namespace SweetEditor {
	...
	internal sealed class DecorationProviderManager {
		...
		private const int TextChangeRefreshDelayMs = 16;
		...
		public void OnTextChanged(IReadOnlyList<TextChange>? changes) => ScheduleRefresh(TextChangeRefreshDelayMs, changes);
		...
		private void ApplySpanMode(SpanLayer layer, DecorationApplyMode mode) {
			if (mode == DecorationApplyMode.REPLACE_ALL) {
				...
				ClearSpanRange(layer, lastContextStartLine, lastContextEndLine);
			}
		}
		...
		private void ApplyInlayMode(DecorationApplyMode mode) {
			if (mode == DecorationApplyMode.REPLACE_ALL) {
				...
				ClearInlayRange(lastContextStartLine, lastContextEndLine);
			}
		}
		...
		private void ApplyDiagnosticMode(DecorationApplyMode mode) {
			if (mode == DecorationApplyMode.REPLACE_ALL) {
				...
				ClearDiagnosticRange(lastContextStartLine, lastContextEndLine);
			}
		}
		...
		private void ApplyGutterMode(DecorationApplyMode mode) {
			if (mode == DecorationApplyMode.REPLACE_ALL) {
				...
				ClearGutterRange(lastContextStartLine, lastContextEndLine);
			}
		}
		...
		private void ApplyPhantomMode(DecorationApplyMode mode) {
			if (mode == DecorationApplyMode.REPLACE_ALL) {
				...
				ClearPhantomRange(lastContextStartLine, lastContextEndLine);
			}
		}
