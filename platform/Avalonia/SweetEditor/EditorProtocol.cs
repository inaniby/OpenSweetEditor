		...
		internal static byte[] PackLinkedEditingPayload(LinkedEditingModel model) {
			...
			for (int groupOrdinal = 0; groupOrdinal < groupCount; groupOrdinal++) {
				...
				foreach (var range in group.Ranges) {
					...
					WriteInt32LE(payload, ref offset, range.Start.Line);
					WriteInt32LE(payload, ref offset, range.Start.Column);
					WriteInt32LE(payload, ref offset, range.End.Line);
					WriteInt32LE(payload, ref offset, range.End.Column);
				}
			}
