...
| **Copilot** *(SHOULD)* | `InlineSuggestion`、宿主可见的 accept/dismiss 回调机制；MAY: `InlineSuggestionListener`、`InlineSuggestionController` | 内联建议数据 + 回调；Controller 为可选实现方式 |
| **Selection** *(MAY, 仅移动端)* | `SelectionMenuController`、`SelectionMenuItem`、宿主可见的点击回调机制；MAY: `SelectionMenuListener` | 选区菜单（桌面端 MAY 省略） |
...
> 事件系统如果采用平台原生 `event` / delegate / stream / signal 等机制，可以不暴露 `EditorEventBus` / `EditorEventListener` 公共类型；此时只要求其语义满足第 7 节。
...
> 事件暴露方式不要求统一方法名。平台 MUST 按第 7 节提供类型安全事件机制，可以采用 `subscribe` / `unsubscribe`、平台原生 `event` / delegate、typed `Stream` getter、signal / observer 等等效形式。

> 第 3.2 节是控件入口层的公共 API 索引。可选模块的公共 API 和回调契约会在后续章节继续补充定义（例如第 4、7、15、21 节）。
...
| 控件销毁时清理 | **MUST** | 控件销毁时 MUST 自动注销所有已注册的 Provider，并取消或标记所有进行中的异步请求为过期，使延迟结果被忽略 |
...
| `setInlineSuggestionCallbacks(callbacks)` | **MUST** | 注册宿主可见的 accepted / dismissed 回调 |

> 平台 MAY 暴露语义等价的 API，例如 `setInlineSuggestionListener(listener)`、delegate setter、事件订阅或 typed stream。
...
## 21. Selection / SelectionMenu 接口定义（MAY，仅移动端）

Selection menu 模块整体为 MAY 级别。桌面平台 MAY 完全省略；如果实现，则 MUST 遵循以下契约。

### 21.1 `SelectionMenuItem` 数据类型
...
### 21.2 Selection Menu 回调契约

平台 MUST 提供宿主可见的方式来观察选区菜单项被触发。平台 MAY 采用显式 listener 接口、delegate / closure / callback setter，或平台原生事件 / typed stream / signal。
...
该回调契约 MUST 同时满足以下条件：
- 一个可见的选区菜单 MUST 暴露宿主可见的 `item-selected` 事件和 `dismissed` 事件
- `item-selected` 的 payload MUST 包含被点击的 `itemId`，或其他能无歧义标识该菜单项的等价 payload
- 对于同一个已展示的菜单实例，`dismissed` 最多触发一次
- 菜单实例一旦被关闭，后续不得再发出该实例的任何选区菜单回调
- 在 widget 销毁、unbind 或 controller dispose 之后，不得再发出任何宿主可见的选区菜单回调

### 21.3 `SweetEditor` Selection API

| 方法 | 约束级别 | 说明 |
|---|---|---|
| `showSelectionMenu(items)` | **MUST** | 显示锚定到当前选区或光标几何位置的选区菜单 |
| `dismissSelectionMenu()` | **MUST** | 关闭当前可见的选区菜单 |
| `isSelectionMenuShowing()` | **SHOULD** | 查询当前是否有选区菜单可见 |
| `setSelectionMenuCallbacks(callbacks)` | **SHOULD** | 注册宿主可见的 item-selected / dismissed 回调 |

> 平台 MAY 暴露语义等价的 API，例如 `setSelectionMenuListener(listener)`、delegate setter、事件订阅或 typed stream。
